...
from agents.models.cp3er_networks_moe import CPActor as Actor
from agents.models.cp3er_networks_moe import MoGCritic as Critic
from agents.models.cp3er_networks_moe import Encoder
...
class CP3ERAgent:
    ...
    def update_actor(self, obs, action, step):
        ...
        new_action, aux_loss = self.actor(obs)
        ...
        critic_buffer_info = self.critic(obs, action)
        mus, stdevs, logits = critic_buffer_info['mus'], critic_buffer_info['stdevs'], critic_buffer_info['logits']
        critic_buffer_dist = self.to_distribution(mus, stdevs, logits)

        q_estimate_buffer = critic_buffer_dist.mean

        cm_loss = self.actor.cm.consistency_losses(action, obs)
        bc_loss = cm_loss.mean() + (cm_loss * (q_estimate_buffer > q_estimate).float().detach()).mean() * 10
        actor_loss = q_loss + 0.05 * bc_loss + aux_loss
